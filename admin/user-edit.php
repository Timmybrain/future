...
if ($_GET['user'] != 'new') {
  ...
  $user = $f->fetch_author($nick);
  $id = $user->author_id;
  $the_message = "Account details successfully updated!";
}
else {
  $id = $_GET['user'];
  $the_message = "New Account Created!";
}
?>
<div class="main-grid">
      ...
      <div class="col-md-9 personal-info">
        ...
        <form action="<?=$_SERVER['PHP_SELF']?>" method="post" class="form-horizontal" role="form">
          ...
          <div class="form-group">
            ...
            <div class="col-md-8">
              <input name="username" class="form-control" type="text" value="<?=@$user->author_nick?>" <?= ($id != "new") ? "disabled" : "" ?> >
            </div>
          </div>
          <?php
          if ($id === 'new') {
